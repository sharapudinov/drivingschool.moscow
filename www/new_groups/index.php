<?
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetTitle("Набор в новые группы");
?>

    <div class="pbs-main-wrapper" style="overflow-x: visible;"><p>Информация регулярно обновляется.</p>
        <div class="pbs-row" data-width="full-width-retain-content" style="min-height: 200px; position: relative; padding-top: 0px !important; padding-bottom: 0px !important;  margin-left: 0px; margin-right: 0px; ">
            <div class="pbs-col" style="flex-basis: 50%; border: 5px solid #fff; padding: 0px !important; magrin: 0px !important; magrin-left: 0px !important;">
                <table bgcolor="#f4f4f4" style="border-style: hidden !important;">
                    <tbody>
                    <tr style="border-style: hidden !important;">
                        <td colspan="3" style="text-align:center; border-style: hidden !important;">  <strong><p></p>
                                <h3>Проспект Вернадского</h3>
                            </strong><p><strong></strong></p>
                            <p><strong><i class="fa fa-phone-square" aria-hidden="true"></i> +7 (495) 740 76 14</strong></p>
                        </td>
                    </tr>
                    <tr bgcolor="#fff" style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  <strong>Начало</strong>  </td>
                        <td style="border-style: hidden !important;">  <strong>Теория</strong>  </td>
                        <td style="border-style: hidden !important;">  <strong>Время</strong>  </td>
                    </tr>
                    <tr style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    <tr bgcolor="#fff" style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    <tr style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    </tbody>
                </table>
            </div>
            <div class="pbs-col" style="flex-basis: 50%; border: 5px solid #fff; padding: 0px !important; magrin: 0px !important;">
                <table bgcolor="#f4f4f4" style="border-style: hidden !important;">
                    <tbody>
                    <tr style="border-style: hidden !important;">
                        <td colspan="3" style="text-align:center; border-style: hidden !important;">  <strong><p></p>
                                <h3>Очаково-Матвеевское</h3>
                            </strong><p><strong></strong></p>
                            <p><strong><i class="fa fa-phone-square" aria-hidden="true"></i> +7 (495) 782 30 25</strong></p>
                        </td>
                    </tr>
                    <tr bgcolor="#fff" style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  <strong>Начало</strong>  </td>
                        <td style="border-style: hidden !important;">  <strong>Теория</strong>  </td>
                        <td style="border-style: hidden !important;">  <strong>Время</strong>  </td>
                    </tr>
                    <tr style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    <tr bgcolor="#fff" style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    <tr style="border-style: hidden !important;">
                        <td style="border-style: hidden !important;">  21 февраля  </td>
                        <td style="border-style: hidden !important;">  вторник и четверг  </td>
                        <td style="border-style: hidden !important;">  с 18:30 до 21:00  </td>
                    </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <p><i class="fa fa-car" aria-hidden="true"></i> Практические занятия проводятся по индивидуальному графику.</p>
        <div class="pbs-row" data-width="full-width" style="min-height: 582px; position: relative; margin-top: 55px;  margin-left: 0px; margin-right: 0px;">
            <div class="pbs-col" style="flex-basis: 15.1777%; padding-right: 8px;"></div>
            <div class="pbs-col" style="flex-basis: 57.9555%; padding-left: 0px; padding-right: 0px; padding-bottom: 0px; background-color: rgb(255, 255, 255);">
                <p><img src="/upload/images/page/Оставьте-заявку-1.png" alt="Оставьте-заявку-на-обучение-в-автошколу" width="300" height="181" class="alignnone size-full wp-image-293"><br>
                </p><div role="form" class="wpcf7" id="wpcf7-f268-p213-o1" lang="ru-RU" dir="ltr">
                    <div class="screen-reader-response"></div>
                    <form action="/novie-gruppi/#wpcf7-f268-p213-o1" method="post" class="wpcf7-form" novalidate="novalidate">
                        <div style="display: none;">
                            <input type="hidden" name="_wpcf7" value="268">
                            <input type="hidden" name="_wpcf7_version" value="4.7">
                            <input type="hidden" name="_wpcf7_locale" value="ru_RU">
                            <input type="hidden" name="_wpcf7_unit_tag" value="wpcf7-f268-p213-o1">
                            <input type="hidden" name="_wpnonce" value="f0f693cd9a">
                        </div>
                        <p><label> <b>Ваше имя</b><br>
                                <span class="wpcf7-form-control-wrap your-name"><input type="text" name="your-name" value="" size="40" class="wpcf7-form-control wpcf7-text wpcf7-validates-as-required" aria-required="true" aria-invalid="false"></span> </label></p>
                        <p><label> <b>Ваш e-mail</b> (для учебных материалов)<br>
                                <span class="wpcf7-form-control-wrap your-email"><input type="email" name="your-email" value="" size="40" class="wpcf7-form-control wpcf7-text wpcf7-email wpcf7-validates-as-required wpcf7-validates-as-email" aria-required="true" aria-invalid="false"></span> </label></p>
                        <p><label> <b>Ваш телефон</b><br>
                                <span class="wpcf7-form-control-wrap your-subject"><input type="text" name="your-subject" value="" size="40" class="wpcf7-form-control wpcf7-text" aria-invalid="false"></span> </label></p>
                        <p><label> <b>Пожелания</b> относительно филиала, графика обучения и т.д.<br>
                                <span class="wpcf7-form-control-wrap your-message"><textarea name="your-message" cols="40" rows="10" class="wpcf7-form-control wpcf7-textarea" aria-invalid="false"></textarea></span> </label></p>
                        <p><input type="submit" value="Отправить" class="wpcf7-form-control wpcf7-submit"><span class="ajax-loader"></span></p>
                        <div class="wpcf7-response-output wpcf7-display-none"></div></form></div></div>
            <div class="pbs-col" style="flex-basis: 26.8668%;"></div>
        </div>
    </div>
<?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>